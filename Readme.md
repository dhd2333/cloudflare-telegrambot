...
6. **注册 Webhook**
   - 访问 `https://your-worker-name.your-account.workers.dev/registerWebhook`（访问自己的https://xxx.workers.dev/registerWebhook，不是复制）
   - 看到 "Ok" 表示注册成功
...
2. **管理命令**
   ...
   - `/block`：屏蔽用户（回复用户消息后使用）
   - `/unblock`：解除屏蔽（回复用户消息后使用）
   - `/checkblock`：检查用户屏蔽状态
   - `/broadcast`：广播消息（回复要广播的消息后使用）
